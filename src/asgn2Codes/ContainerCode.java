...
public class ContainerCode {
	...
	public ContainerCode(String code) throws InvalidCodeException {
		...
		if (code.length() != 11)
		{
			throw new InvalidCodeException("Container Code is Wrong Length");	
		}
		
		if (Character.isLowerCase(code.charAt(0)) || 
				Character.isLowerCase(code.charAt(1)) || 
				Character.isLowerCase(code.charAt(2)))
		{
			throw new InvalidCodeException("Owner Code is not capitals");
		}
		
		if (code.charAt(3) != 'U')
		{
			throw new InvalidCodeException("Type is not freight (U)");
		}
		
		if (!(Character.isDigit(code.charAt(4)) &&
				Character.isDigit(code.charAt(5)) &&
				Character.isDigit(code.charAt(6)) &&
				Character.isDigit(code.charAt(7)) &&
				Character.isDigit(code.charAt(8)) &&
				Character.isDigit(code.charAt(9))))
		{
			throw new InvalidCodeException("Serial is not all numbers");
		}
		
		if (checkCode(code)!= Character.getNumericValue(code.charAt(10)))
		{
			throw new InvalidCodeException("Invalid check value");
		}
	...
	/**
	 * Returns the integer value of the check value for a given container code
	 * 
	 * @param code a Container Code
	 * @Return int of the check value for given container code
	 */
	private int checkCode(String code)
	{
		return (((int) code.charAt(0)) + 
				...
				Character.getNumericValue(code.charAt(9)) ) % 10;
	}
