...
public class CargoManifest {
	 ...
	 * parameters
	 */
	private Integer mStacks, mHeight, mWeight, cWeight = 0;
	private FreightContainer[][] shipManifest;
	...
	throws ManifestException {
		...
		{
			throw new ManifestException("Cargo Manifest values are not valid, all must be greater then 0");
		}
		
		mStacks = numStacks;
		mHeight = maxHeight;
		mWeight = maxWeight;
		shipManifest = new FreightContainer[mStacks][mHeight];
	}
	...
	public void loadContainer(FreightContainer newContainer) throws ManifestException {
		...
		boolean success = false; //Value to determine if Container has been placed into the manifest
		
		if (cWeight + newContainer.getGrossWeight() > mWeight) //Check if weight limits allow the selected container to be added to the ship
		{
			throw new ManifestException("Container would cause max weight to be exceeded");
		}
		if ((mStacks == 0) || (mHeight == 0)){ //Check if container limits allow any containers on ship
			throw new ManifestException("Containers cannot be added, max stacks or max height 0.");
		}
		
		for (int i = 0; (i <= mStacks && success == false); i++ ){ //Search manifest stacks starting nearest to bridge
			if (shipManifest[i][0] == null){ //If stack empty, start new stack with noewContainer
				shipManifest[i][0] = newContainer;
				cWeight = cWeight + newContainer.getGrossWeight();
				success = true;
				break; //break
			}else{ 
				if ((shipManifest[i][0].getClass() == newContainer.getClass()) && (mHeight > 1)){ //If stack not empty check if newContainer of same class as existing container and check that more then one container may be placed in stack
					for (int j = 1; (j <= mHeight && success == false); j++){ //Search stack for free place and assign new container if found.
						if (shipManifest[i][j] == null) {
							shipManifest[i][j] = newContainer;
							cWeight = cWeight + newContainer.getGrossWeight();
							success = true;
							break;
						}
		...
		if (success == false){ //If container not placed in manifest throw exception
			throw new ManifestException("No valid position for container");
		}
	}
	...
	public void unloadContainer(ContainerCode containerId) throws ManifestException {
		...
		for (int i = 0; i <= mStacks; i++){
			for (int j = 0; j <= mHeight; j++){
				if (shipManifest[i][j].toString() == containerId.toString()){
					if (j == mHeight){
						shipManifest[i][j] = null;
						cWeight = cWeight - shipManifest[i][j].getGrossWeight();
					}else if (shipManifest[i][j+1] == null){
						shipManifest[i][j] = null;
						cWeight = cWeight - shipManifest[i][j].getGrossWeight();
					}else{
						throw new ManifestException("Cargo not at top of stack");
					}
				}
			}
		}
		...
		if (success == false){
			throw new ManifestException("Cargo not found in manifest");
		}
	}
	...
	public Integer whichStack(ContainerCode queryContainer) {
		...
		for (int i = 0; i <= mStacks; i++){
			for (int j = 0; j <= mHeight; j++){
				if (shipManifest[i][j].toString() == queryContainer.toString()){
					return i;
				}
			}
		}
	...
	public FreightContainer[] toArray(Integer stackNo) throws ManifestException {
		...
		if (stackNo <= mStacks){
		return shipManifest[stackNo];	
		}else{
			throw new ManifestException("Invalid Stack Number");
		}
	}
	...
	public String toString(ContainerCode toFind) {
		...
		String toReturn = "";
		for (int i = 0; i < shipManifest.length; ++i) {
			FreightContainer[] currentStack = shipManifest[i];
			toReturn += "|";
			for (int j = 0; j < currentStack.length; ++j) {
				if (toFind != null && currentStack[j].getCode().equals(toFind))
					toReturn += "|*" + currentStack[j].getCode().toString() + "*|";
				else
					toReturn += "| " + currentStack[j].getCode().toString() + " |";
			}
